...
def run_injection(d_psrs, cwpars, cwcommon, pulsterm=False, seed=345):
    """
    Parameters
    ----------
    ...
    cgw_params_inj = {}
    for psr in d_psrs:
        cgw_params_inj[psr.name + '_cw_d_psr'] = 0.
    for name, par in zip(cwcommon, cwpars):
        cgw_params_inj[name] = par

    _, residuals = model.sample(jax.random.key(seed), cgw_params_inj)
